...
rm -f survival_results_all_EXP_all_OUT.csv
rm -f survival_results_all_EXP_all_OUT.timings
...
do
   ...
   do
      ...
      export OUTPUT="survival_results_${exposure}_EXP_${outcome}_OUT.csv"
      export TIMINGS="survival_results_${exposure}_EXP_${outcome}_OUT.timings"
      # exposure variables which are same for all exposure-outcome pairs
      export INPUT_DEFINITIONS="X_input_definitions.csv"
      export INPUT_INFO="X_input_info.csv"
      ...
      if [ ! -s survival_results_all_EXP_all_OUT.csv ]
      then
         cp survival_results_${exposure}_EXP_${outcome}_OUT.csv survival_results_all_EXP_all_OUT.csv
         cp survival_results_${exposure}_EXP_${outcome}_OUT.timings survival_results_all_EXP_all_OUT.timings
      else
         tail -n +2 survival_results_${exposure}_EXP_${outcome}_OUT.csv >> survival_results_all_EXP_all_OUT.csv
         tail -n +2 survival_results_${exposure}_EXP_${outcome}_OUT.timings >> survival_results_all_EXP_all_OUT.timings
      fi
